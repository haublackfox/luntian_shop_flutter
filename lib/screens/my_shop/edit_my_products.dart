...
class EditProductForm extends StatefulWidget {
  const EditProductForm(
      {Key? key, required String product_id, required String shop_id})
  ...
  static const routeName = '/EditProductForm';
...
//   var _productBrand = '';
  ...
  void getData() async {
    ...
    final DocumentSnapshot userDoc = await FirebaseFirestore.instance
        .collection('product')
        .doc(widget._product_id)
        .get();

    try {
      setState(() {
        _categoryValue = userDoc.get('category');
        _productTitle = userDoc.get('name');
        _productDescription = userDoc.get('description');
        _productPrice = userDoc.get('price');
        _productQuantity = userDoc.get('quantity');
        _productId = userDoc.get('id');
        url = userDoc.get('image_url');
      });

      print("IMAGEURLLLLLLLLLLL");
      print(url);
      print("IMAGEURLLLLLLLLLLL");
      _nameController.text = _productTitle;
      _priceController.text = _productPrice;
      _categoryController.text = _categoryValue;
      _descriptionController.text = _productDescription;
      _quantityController.text = _productQuantity;
      _categoryController.text = _categoryValue;
    } catch (e) {
      print("Error");
      setState(() {
    ...
    if (isValid) {
      ...
      try {
        ...
        final ref = FirebaseStorage.instance
            ...
            .child(_productId + now.toString() + '.jpg');
        ...
        showSnackBar(context, "Product details has been successfully updated",
            Colors.green, 3000);

        Navigator.push(
          context,
          MaterialPageRoute(
              builder: (context) => MyProducts(shop_id: widget._shop_id)),
        );
